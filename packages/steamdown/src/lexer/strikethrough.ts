...
export default class Strikethrough extends TaggedInlineToken {
  ...
  public static lex(md: string): [token: Strikethrough, remainder: string] | null {
    const escapedDelim = "\\~\\~";
    const scan = InlineToken.scan(md.slice(2), escapedDelim);
    if (!scan) {
      return null;
    }
    ...
    return [new Strikethrough(text, literal), remainder];
  }
}
