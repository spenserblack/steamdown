 ...
 */
const renderSimpleBlock = (tag: string, content: string): string => `[${tag}]\n${content}\n[/${tag}]`;

const renderInlineNodes = (nodes: nodes.Inline[], context: Context): string => nodes.map<string>((node) => {
  switch (node.type) {
    case "text":
      return node.text;
    case "escaped":
    ...
    case "id-url": {
      ...
      const content = node.nodes != null ? renderInlineNodes(node.nodes, context) : node.id;
...
const renderList = (list: nodes.List, context: Context): string => {
  ...
  const items = list.items.map((item) => `  [*]${renderBlocks(item.nodes, context)}`).join("\n");
  return `[${tag}]\n${items}\n[/${tag}]`;
};

const renderTableRow = (row: nodes.TableRow, context: Context, cellTag: "td" | "th"): string => {
  const renderedCells = row.cells.map((cell) => `    [${cellTag}]${renderInlineNodes(cell.nodes, context)}[/${cellTag}]`).join("\n");
  return `  [tr]\n${renderedCells}\n  [/tr]`;
};
...
const renderBlocks = (blocks: nodes.Block[], context: Context): string => blocks.map((node) => {
  switch (node.type) {
    ...
    case "quote": {
      ...
      const openTag = author != null ? `[quote=${author}${postId != null ? `;${postId}` : ""}]` : "[quote]";
...
}).filter((rendered) => rendered != null).join("\n\n");

/**
 * Renders the syntax tree into Steam's markup.
 */
export const render = (root: nodes.Root, context: Context): string => renderBlocks(root.nodes, context);
